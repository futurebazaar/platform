...
public class QuantiumOutboundImpl implements ShipmentOutbound {
	...
	private void loadProperties() {
		try {
			...
			serverFilePathFtp = prop.getProperty("future.server.outbound.quantium.ftp");
			serverFilePathMail = prop.getProperty("future.server.outbound.quantium.mail");
			uploadedPath = prop.getProperty("future.server.outbound.quantium.ftp.uploaded");
			mailSentPath = prop.getProperty("future.server.outbound.quantium.mail.sent");
			prop.clear();
	...
	public void generateFile(List<ParcelItem> parcelItemList) {
		...
		String extension = prop.getProperty("quantium.outbound.extension");
		String fileNamePrefix = prop.getProperty("quantium.outbound.fileNamePrefix");
		String fileNameFormat = prop.getProperty("quantium.outbound.fileNameFormat");
	...
	public void uploadOutboundFile() {
		String uploadPath = prop.getProperty("quantium.ftp.outbound");
	...
	private MailTO createMail(List<File> attachments) {
		...
		String[] to = prop.getProperty("quantium.ftp.to").split(",");
		String[] cc = null;
		if(StringUtils.isNotBlank(prop.getProperty("quantium.ftp.cc"))) {
			cc = prop.getProperty("quantium.ftp.cc").split(",");
		}
		String[] bcc = null;
		if(StringUtils.isNotBlank(prop.getProperty("quantium.ftp.bcc"))) {
			bcc = prop.getProperty("quantium.ftp.bcc").split(",");
		}
		ftpMail.setFrom(prop.getProperty("quantium.ftp.from"));
		DateTime dateTime = new DateTime();
		ftpMail.setMessage(prop.getProperty("quantium.ftp.message").replace("%date%", dateTime.toString()));
		ftpMail.setSubject(prop.getProperty("quantium.ftp.subject").replace("%date%", dateTime.toString()));
