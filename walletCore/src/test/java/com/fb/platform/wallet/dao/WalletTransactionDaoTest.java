...
public class WalletTransactionDaoTest extends BaseTestCase {
	...
	public void wrongWalletRefundIdTransaction(){
		...
		assertNotNull(wallet);
		assertNotNull(wallet.getId());
		WalletTransaction walletTransaction = wallet.credit(new Money(new BigDecimal("200.00")), SubWalletType.REFUND, 0, 1, 0);
	...
	public void reverseTransactionTest(){
		...
		WalletTransaction walletTransactionNew = wallet4.reverseTransaction(debitWalletTranFetch,debitWalletTranFetch.getAmount(),new Money(new BigDecimal("0.00")),0);
