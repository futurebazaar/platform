...
public class PointsDaoJdbcImpl implements PointsDao {
	...
	private static final String INSERT_POINTS_HEADER_SQL = "INSERT INTO points_header "
			+ "(order_id, "
			+ "reference_id, "
			+ "loyalty_card, "
			+ "partner_merchant_id, "
	...
	private static final String UPDATE_STATUS_SQL = "UPDATE points_header SET "
			+ "txn_timestamp = ?, " + "status = 'DONE' WHERE "
			+ "txn_action_code = ? AND " + "settlement_date = ? AND "
			+ "partner_merchant_id = ? ";

	private static final String LOAD_POINTS_HEADER_SQL = " SELECT * FROM points_header WHERE "
			+ "txn_action_code = ? AND "
			+ "order_id = ? AND "
			+ "txn_classification_code = ? " + "order by id desc";

	private static final String LOAD_HEADER_DATA_QUERY = "SELECT * FROM "
			+ "points_header WHERE " + "status = 'FRESH' AND "
			+ "txn_action_code = ? AND " + "settlement_date = ? AND "
			+ "partner_merchant_id = ?  " + "order by id ";

	private static final String LOAD_EARN_DATA_QUERY = "SELECT * FROM "
			+ "points_items " + "WHERE points_header_id = ? "
			+ "order by id desc";

	private static final String INSERT_POINTS_ITEMS_SQL = "INSERT INTO points_items "
			+ "(points_header_id, "
			+ "quantity, "
			+ "department_code, "
			+ "department_name, "
	...
	public Long insertPointsHeaderData(final PointsHeader pointsHeader) {
		logger.info("Inserting Points Header data for order Id : "
				+ pointsHeader.getOrderId());
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con)
				...
				PreparedStatement ps = con.prepareStatement(
						INSERT_POINTS_HEADER_SQL, new String[] { "id" });
				...
				ps.setDate(9, new java.sql.Date(pointsHeader.getTxnDate()
						.getMillis()));
				ps.setDate(10, new java.sql.Date(pointsHeader
						.getSettlementDate().getMillis()));
				ps.setTimestamp(11, new java.sql.Timestamp(pointsHeader
						.getTxnTimestamp().getMillis()));
	...
	@Override
	public void updateStatus(final String txnActionCode,
			final String settlementDate, final String merchantId) {
		logger.info("Updating Points Header data for actionCode : "
				+ txnActionCode + " for merchant : " + merchantId
				+ " on date : " + settlementDate);
		...
		if (rowUpdated <= 0) {
			throw new PlatformException(
					"Unable to update the status for merchant : " + merchantId
							+ " with txnActionCode : " + txnActionCode
							+ " on settlement_date : " + settlementDate);
		}
	}
	...
	public PointsHeader getHeaderDetails(long orderId, String txnActionCode,
		...
		return jdbcTemplate.queryForObject(LOAD_POINTS_HEADER_SQL,
				new Object[] { txnActionCode, orderId, txnClassificationCode },
				new HeaderMapper());
	}
	...
	public Collection<PointsHeader> loadPointsHeaderData(String txnActionCode,
		...
		Collection<PointsHeader> pointsHeaderList = jdbcTemplate.query(
				LOAD_HEADER_DATA_QUERY, new Object[] { txnActionCode,
						settlementDate, merchantId }, new HeaderMapper());
		return pointsHeaderList;
	}

	private class HeaderMapper implements RowMapper<PointsHeader> {

		public PointsHeader mapRow(ResultSet rs, int rowNum)
				throws SQLException {
			...
			pointsHeader.setTxnTimestamp(new DateTime(rs.getTimestamp(
					"txn_timestamp").getTime()));
			pointsHeader.setSettlementDate(new DateTime(rs.getDate(
					"settlement_date").getTime()));
			pointsHeader.setTxnDate(new DateTime(rs.getDate("txn_date")
					.getTime()));
			...
			pointsHeader.setPartnerMerchantId(rs
					.getString("partner_merchant_id"));
			pointsHeader.setPartnerTerminalId(rs
					.getString("partner_terminal_id"));
			pointsHeader.setReason(rs.getString("reason"));
			pointsHeader.setReferenceId(rs.getString("reference_id"));
			pointsHeader.setTxnClassificationCode(rs
					.getString("txn_classification_code"));
	...
	public Collection<PointsItems> loadPointsItemData(long pointsHeaderId) {
		logger.info("Getting Points Item data for points header Id : "
				+ pointsHeaderId);
		Collection<PointsItems> earnPointsList = jdbcTemplate.query(
				LOAD_EARN_DATA_QUERY, new Object[] { pointsHeaderId },
				new EarnDataMapper());
	...
	public void insertPointsItemsData(final OrderItemRequest itemRequest,
		...
		logger.info("Inserting Points Item data for header Id : " + headerId
				+ " and itemId : " + itemRequest.getId());
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con)
					throws SQLException {
				PreparedStatement ps = con.prepareStatement(
						INSERT_POINTS_ITEMS_SQL, new String[] { "id" });
