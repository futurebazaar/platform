...
public class RuleCacheAccess extends AbstractCacheAccess {
	...
	public void put(String ruleName, PointsRule rule) {
		platformCachingManager
				.put(NamedCachesEnum.POINTS_CACHE, ruleName, rule);
	}

	public PointsRule get(String ruleName) {
		Object rule = platformCachingManager.get(NamedCachesEnum.POINTS_CACHE,
				ruleName);
		if (rule != null) {
	...
	public boolean clear(String ruleName) {
		return platformCachingManager.remove(NamedCachesEnum.POINTS_CACHE,
				ruleName);
	}
