...
public class BuyWorthXGetYPercentOffRuleImpl implements PromotionRule,
	...
	private static transient Log log = LogFactory
			.getLog(BuyWorthXGetYPercentOffRuleImpl.class);
	...
	public void init(RuleConfiguration ruleConfig) {
		...
		data = (BuyWorthXGetYPercentOffRuleData) RulesEnum.BUY_WORTH_X_GET_Y_PERCENT_OFF
	...
	public PromotionStatusEnum isApplicable(OrderRequest request, int userId,
		...
		if (log.isDebugEnabled()) {
			log.debug("Checking if BuyWorthXGetYPercentOffRuleImpl applies on order : "
					+ request.getOrderId());
		}
	...
	public OrderDiscount execute(OrderDiscount orderDiscount) {
		...
		Money orderVal = request.getOrderValueForRelevantProducts(
				data.getBrands(), data.getIncludeCategoryList(),
				data.getExcludeCategoryList());
		Money discountCalculated = (orderVal.times(data.getDiscountPercentage()
				.doubleValue())).div(100);
