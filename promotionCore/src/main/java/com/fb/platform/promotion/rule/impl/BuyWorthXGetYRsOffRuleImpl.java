...
public class BuyWorthXGetYRsOffRuleImpl implements PromotionRule, Serializable {
	...
	public void init(RuleConfiguration ruleConfig) {
		...
		data = (BuyWorthXGetYRsOffRuleData) RulesEnum.BUY_WORTH_X_GET_Y_RS_OFF
	...
	public PromotionStatusEnum isApplicable(OrderRequest request, int userId,
		...
		if (log.isDebugEnabled()) {
			log.debug("Checking if BuyWorthXGetYRsOffRuleImpl applies on order : "
					+ request.getOrderId());
		}
	...
	public OrderDiscount execute(OrderDiscount orderDiscount) {
		...
		if (log.isDebugEnabled()) {
			log.debug("Executing BuyWorthXGetYRsOffRuleImpl on order : "
					+ request.getOrderId());
		}
