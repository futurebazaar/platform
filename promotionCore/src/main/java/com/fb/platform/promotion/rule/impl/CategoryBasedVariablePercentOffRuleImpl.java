...
public class CategoryBasedVariablePercentOffRuleImpl implements PromotionRule, Serializable {
	...
	public PromotionStatusEnum isApplicable(OrderRequest request, int userId,
		...
		Money orderValue = request.getOrderValueForRelevantProducts(null, data
				.getCategoryDiscountPairs().getAllCategoryList(), null, null);
		if (ListUtil.isValidList(data.getCategoryDiscountPairs().getAllCategoryList())
				&& !request.isAnyProductInCategory(data.getCategoryDiscountPairs()
						.getAllCategoryList())) {
	...
	public OrderDiscount execute(OrderDiscount orderDiscount) {
		...
		for (CategoryDiscountPair pair : data.getCategoryDiscountPairs().getCategoryDiscountMap()) {
			Money orderVal = request.getOrderValueForRelevantProducts(null, pair.getCategoryList(),
					null, null);
			...
			orderDiscount.distributeDiscountOnOrder(orderDiscount, null, pair.getCategoryList(),
					null, null);
		}
	...
	public List<RuleConfigItemDescriptor> getRuleConfigs() {
		...
		ruleConfigs.add(new RuleConfigItemDescriptor(RuleConfigDescriptorEnum.MIN_ORDER_VALUE,
				false));
		ruleConfigs.add(new RuleConfigItemDescriptor(RuleConfigDescriptorEnum.DISCOUNT_PERCENTAGE,
				true, true));
		ruleConfigs.add(new RuleConfigItemDescriptor(
				RuleConfigDescriptorEnum.CATEGORY_INCLUDE_LIST, true, true));
