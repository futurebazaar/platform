...
public class OrderRequest implements Serializable {
	...
	public Money getOrderValueForRelevantProducts(List<Integer> brandList, List<Integer> catIncludeList,
		List<Integer> catExcludeList, List<Integer> sellerList) {
		Money orderValueForRelevantProducts = new Money(new BigDecimal(0));
		for (OrderItem o : orderItems) {
			if ((!ListUtil.isValidList(brandList) || o.isOrderItemInBrand(brandList))
				...
				&& (!ListUtil.isValidList(sellerList) || !o.isOrderItemOfSeller(sellerList))) {
				orderValueForRelevantProducts = orderValueForRelevantProducts.plus(new Money(o.getPrice()));
			}
		}
		return orderValueForRelevantProducts;
	}
	...
	public int getOrderQuantityForRelevantProducts(List<Integer> brandList, List<Integer> catIncludeList,
		List<Integer> catExcludeList, List<Integer> sellerList) {
		int count = 0;
		for (OrderItem o : orderItems) {
			if ((!ListUtil.isValidList(brandList) || o.isOrderItemInBrand(brandList))
	...
	public boolean isAnyProductOfSeller(List<Integer> sellers) {
		for (OrderItem o : orderItems) {
			System.out.println(o.isOrderItemOfSeller(sellers));
			if (o.isOrderItemOfSeller(sellers)) {
				return true;
			}
		}
		return false;
	...
	public Money getOrderValueForSellerProducts(List<Integer> sellers) {
		Money orderValueForSellerProducts = new Money(new BigDecimal(0));
		for (OrderItem o : orderItems) {
			if (o.isOrderItemOfSeller(sellers)) {
				orderValueForSellerProducts = orderValueForSellerProducts.plus(new Money(o.getPrice()));
			}
		}
		return orderValueForSellerProducts;
	}
