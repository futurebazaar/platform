...
public class BuyXBrandGetYRsOffOnZProductRuleImpl implements PromotionRule,
	...
	private static transient Log log = LogFactory
			.getLog(BuyXBrandGetYRsOffOnZProductRuleImpl.class);
	...
	public void init(RuleConfiguration ruleConfig) {
		minOrderValue = new Money(BigDecimal.valueOf(Double.valueOf(ruleConfig
				.getConfigItemValue(RuleConfigDescriptorEnum.MIN_ORDER_VALUE
						.name()))));
		StrTokenizer strTokCategories = new StrTokenizer(
				ruleConfig.getConfigItemValue(RuleConfigDescriptorEnum.BRAND_LIST
						.name()), ",");
		brands = StringToIntegerList.convert((List<String>) strTokCategories
				.getTokenList());
		if (ruleConfig
				.isConfigItemPresent(RuleConfigDescriptorEnum.CLIENT_LIST)) {
			StrTokenizer strTokClients = new StrTokenizer(
					ruleConfig.getConfigItemValue(RuleConfigDescriptorEnum.CLIENT_LIST
							.name()), ",");
			clientList = StringToIntegerList
					.convert((List<String>) strTokClients.getTokenList());
			log.info("clientList = " + clientList);
		}
		productId = Integer.valueOf(
				ruleConfig
						.getConfigItemValue(RuleConfigDescriptorEnum.PRODUCT_ID
								.name())).intValue();
		productDiscountValue = new Money(
				BigDecimal.valueOf(Double.valueOf(ruleConfig
						.getConfigItemValue(RuleConfigDescriptorEnum.PRODUCT_DISCOUNTED_VALUE
								.name()))));
		log.info("minOrderValue : " + minOrderValue.toString()
				+ ", productId : " + productId + ", productDiscountValue : "
				+ productDiscountValue + ", brandList : " + brands);
	}
	...
	public PromotionStatusEnum isApplicable(OrderRequest request, int userId,
		...
		if (log.isDebugEnabled()) {
			log.debug("Checking if BuyXBrandGetYRsOffOnZProductRuleImpl applies on order : "
					+ request.getOrderId());
		}
		// Money orderValue = new Money(request.getOrderValue());
		if (ListUtil.isValidList(clientList)
				&& !request.isValidClient(clientList)) {
			return PromotionStatusEnum.INVALID_CLIENT;
		}
		/*
		 * if(orderValue.lt(minOrderValue)){ return
	...
	public OrderDiscount execute(OrderDiscount orderDiscount) {
		 ...
		 * if(log.isDebugEnabled()) {
		 * log.debug("Executing BuyXBrandGetYRsOffOnZProductRuleImpl on order : "
		 * + request.getOrderId()); } Money productPrice =
		 * request.getProductPrice(productId); Money zeroMoney = new Money(new
		 * BigDecimal(0)); Money totalDiscountValue = productPrice.eq(zeroMoney)
		 * ? zeroMoney : productPrice.minus(productDiscountValue);
	...
	public RuleConfigDescriptor getRuleConfigs() {
		...
		ruleConfigDescriptor
				.addConfigItemDescriptor(new RuleConfigItemDescriptor(
						RuleConfigDescriptorEnum.BRAND_LIST, true));
		ruleConfigDescriptor
				.addConfigItemDescriptor(new RuleConfigItemDescriptor(
						RuleConfigDescriptorEnum.PRODUCT_ID, true));
		ruleConfigDescriptor
				.addConfigItemDescriptor(new RuleConfigItemDescriptor(
						RuleConfigDescriptorEnum.MIN_ORDER_VALUE, true));
		ruleConfigDescriptor
				.addConfigItemDescriptor(new RuleConfigItemDescriptor(
						RuleConfigDescriptorEnum.PRODUCT_DISCOUNTED_VALUE, true));
