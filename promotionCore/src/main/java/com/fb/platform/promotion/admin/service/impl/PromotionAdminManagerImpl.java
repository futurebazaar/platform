...
public class PromotionAdminManagerImpl implements PromotionAdminManager {
	...
	/*
	 * @see
	 * com.fb.platform.promotion.service.PromotionAdminManager#fetchRules(com
	 * .fb.platform.promotion.to.FetchRuleRequest)
	 */
	@Override
	public FetchRuleResponse fetchRules(FetchRuleRequest fetchRuleRequest) {
		...
		if (fetchRuleRequest == null
				|| StringUtils.isBlank(fetchRuleRequest.getSessionToken())) {
		...
		AuthenticationTO authentication = authenticationService
				.authenticate(fetchRuleRequest.getSessionToken());
		if (authentication == null) {
			log.info("Invalid session token : "
					+ fetchRuleRequest.getSessionToken());
			fetchRuleResponse.setFetchRulesEnum(FetchRulesEnum.NO_SESSION);
			return fetchRuleResponse;
		}
		...
		try {
			...
			List<RulesEnum> rulesList = promotionAdminService
			...
			for (RulesEnum rulesEnum : rulesList) {
				RuleConfigDescriptor ruleConfigDescriptor = PromotionRuleFactory
						.getRuleConfig(rulesEnum);
			...
			fetchRuleResponse.setSessionToken(fetchRuleRequest
					.getSessionToken());
	...
	public CreatePromotionResponse createPromotion(
		...
		String requestInvalidationString = messageTranslatorUtility
				.translate(createPromotionRequest.isValid());

		if (StringUtils.isNotBlank(requestInvalidationString)) {
			log.info("Create promotion request insufficient data : "
					+ requestInvalidationString);
			createPromotionResponse
					.setCreatePromotionEnum(CreatePromotionEnum.INSUFFICIENT_DATA);
		...
		// authenticate the session token and find out the userId
		AuthenticationTO authentication = authenticationService
				.authenticate(createPromotionRequest.getSessionToken());
		if (authentication == null) {
			// invalid session token
			log.info("Invalid session token : "
					+ createPromotionRequest.getSessionToken());
			createPromotionResponse
					.setCreatePromotionEnum(CreatePromotionEnum.NO_SESSION);
			return createPromotionResponse;
		}

		String missingConfigsList = hasValidRuleConfig(createPromotionRequest
				.getPromotion());

		if (!StringUtils.isEmpty(missingConfigsList)) {
			log.info("Create promotion request rule config missing : "
					+ missingConfigsList);
			createPromotionResponse
					.setCreatePromotionEnum(CreatePromotionEnum.RULE_CONFIG_MISSING);
			createPromotionResponse.setErrorCause(missingConfigsList);
			return createPromotionResponse;
		}

		createPromotionResponse.setSessionToken(createPromotionRequest
				.getSessionToken());

		try {
			int promotionId = promotionAdminService
					.createPromotion(createPromotionRequest.getPromotion());
			if (promotionId > 0) {
				createPromotionResponse.setPromotionId(promotionId);
				createPromotionResponse
						.setCreatePromotionEnum(CreatePromotionEnum.SUCCESS);
			}
		} catch (PromotionNameDuplicateException e) {
			log.error("Duplicate promotion name insertion exception.");
			createPromotionResponse
					.setCreatePromotionEnum(CreatePromotionEnum.DUPLICATE_PROMOTION_NAME);
		} catch (PlatformException e) {
			log.error("Internal error while creating new promotion.");
			createPromotionResponse
					.setCreatePromotionEnum(CreatePromotionEnum.INTERNAL_ERROR);
		}
	...
	@Override
	public SearchPromotionResponse searchPromotion(
			SearchPromotionRequest searchPromotionRequest) {

		SearchPromotionResponse searchPromotionResponse = new SearchPromotionResponse();
		String requestInvalidationList = messageTranslatorUtility
				.translate(searchPromotionRequest.isValid());

		if (!StringUtils.isEmpty(requestInvalidationList)) {
			log.info("Cannot search for promotion insufficient data : "
					+ requestInvalidationList);
			searchPromotionResponse
					.setSearchPromotionEnum(SearchPromotionEnum.INSUFFICIENT_DATA);
		...
		// authenticate the session token and find out the userId
		AuthenticationTO authentication = authenticationService
				.authenticate(searchPromotionRequest.getSessionToken());
		if (authentication == null) {
			// invalid session token
			log.info("Invalid session token : "
					+ searchPromotionRequest.getSessionToken());
			searchPromotionResponse
					.setSearchPromotionEnum(SearchPromotionEnum.NO_SESSION);
			return searchPromotionResponse;
		}
		searchPromotionResponse.setSessionToken(searchPromotionRequest
				.getSessionToken());

		int isActive = searchPromotionRequest.isActive() ? 1 : 0;
		try {
			List<PromotionTO> promotionList = promotionAdminService
					.searchPromotion(searchPromotionRequest.getPromotionName(),
							searchPromotionRequest.getValidFrom(),
							searchPromotionRequest.getValidTill(), isActive,
			...
			int promotionCount = promotionAdminService.getPromotionCount(
					searchPromotionRequest.getPromotionName(),
					searchPromotionRequest.getValidFrom(),
					searchPromotionRequest.getValidTill(), isActive);
			searchPromotionResponse.setTotalCount(promotionCount);
			searchPromotionResponse
					.setSearchPromotionEnum(SearchPromotionEnum.SUCCESS);

		} catch (NoDataFoundException e) {
			log.error("No promotion matching search criteria found.");
			searchPromotionResponse
					.setSearchPromotionEnum(SearchPromotionEnum.NO_DATA_FOUND);
		} catch (PlatformException e) {
			log.error("Internal error while searching promotion.");
			searchPromotionResponse
					.setSearchPromotionEnum(SearchPromotionEnum.INTERNAL_ERROR);
		}
	...
	@Override
	public ViewPromotionResponse viewPromotion(
			ViewPromotionRequest viewPromotionRequest) {
		ViewPromotionResponse viewPromotionResponse = new ViewPromotionResponse();

		String requestInvalidationList = messageTranslatorUtility
				.translate(viewPromotionRequest.isValid());

		if (!StringUtils.isEmpty(requestInvalidationList)) {
			log.info("Cannot fetch promotion details, insufficient data : "
					+ requestInvalidationList);
			viewPromotionResponse
					.setViewPromotionEnum(ViewPromotionEnum.INSUFFICIENT_DATA);
		...
		// authenticate the session token and find out the userId
		AuthenticationTO authentication = authenticationService
				.authenticate(viewPromotionRequest.getSessionToken());
		if (authentication == null) {
			// invalid session token
			log.info("Invalid session token : "
					+ viewPromotionRequest.getSessionToken());
			viewPromotionResponse
					.setViewPromotionEnum(ViewPromotionEnum.NO_SESSION);
			return viewPromotionResponse;
		}

		viewPromotionResponse.setSessionToken(viewPromotionRequest
				.getSessionToken());

		PromotionTO promotionCompleteView = null;

		try {
			promotionCompleteView = promotionAdminService
					.viewPromotion(viewPromotionRequest.getPromotionId());

			int couponCount = promotionAdminService
					.promotionCouponCount(viewPromotionRequest.getPromotionId());

			promotionCompleteView.setCouponCount(couponCount);
			viewPromotionResponse
					.setViewPromotionEnum(ViewPromotionEnum.SUCCESS);
			viewPromotionResponse
					.setPromotionCompleteView(promotionCompleteView);

		} catch (PromotionNotFoundException e) {
			log.error("Promotion not found. Promotion Id : "
					+ viewPromotionRequest.getPromotionId());
			viewPromotionResponse
					.setViewPromotionEnum(ViewPromotionEnum.NO_DATA_FOUND);
		} catch (PlatformException e) {
			log.error("Internal error while fetching promotion.");
			viewPromotionResponse
					.setViewPromotionEnum(ViewPromotionEnum.INTERNAL_ERROR);
		}
	...
	@Override
	public UpdatePromotionResponse updatePromotion(
			UpdatePromotionRequest request) {
		UpdatePromotionResponse response = new UpdatePromotionResponse();

		String requestInvalidationList = messageTranslatorUtility
				.translate(request.isValid());

		if (!StringUtils.isEmpty(requestInvalidationList)) {
			log.info("Cannot update promotion, insufficient data : "
					+ requestInvalidationList);
		...
		if (!StringUtils.isEmpty(missingConfigsList)) {
			log.info("Update promotion request rule config missing : "
					+ missingConfigsList);
		...
		try {
			...
			log.error("Update promotion invalid promotion id : "
					+ request.getPromotion().getId(), e);
			...
			log.error("Exception while updating the promotion. Promotion Id : "
					+ request.getPromotion().getId(), e);
	...
	private String hasValidRuleConfig(PromotionTO promotionTo) {
		...
		RuleConfigDescriptor requiredConfigs = PromotionRuleFactory
				.getRuleConfig(RulesEnum.valueOf(promotionTo.getRuleName()));
		...
		for (int i = promotionTo.getConfigItems().size() - 1; i >= 0; i--) {
			RuleConfigItemTO ruleConfigItemTO = promotionTo.getConfigItems()
					.get(i);
			if (StringUtils.isBlank(ruleConfigItemTO.getRuleConfigValue())) {
				promotionTo.getConfigItems().remove(ruleConfigItemTO);
			} else {
				receivedConfigsMap.put(ruleConfigItemTO.getRuleConfigName(),
						ruleConfigItemTO);
			}
		}
		...
		for (RuleConfigItemDescriptor ruleConfigDescriptorItem : requiredConfigs
			...
			if (ruleConfigDescriptorItem.isMandatory()
					&& !receivedConfigsMap.containsKey(ruleConfigDescriptorItem
							.getRuleConfigDescriptorEnum().toString())) {
				missingConfigsList.add(ruleConfigDescriptorItem
						.getRuleConfigDescriptorEnum().toString());
			}
		}

		// TODO : Similar check for RuleConfigBundles too
	...
	public void setPromotionAdminService(
			PromotionAdminService promotionAdminService) {
		this.promotionAdminService = promotionAdminService;
	}

	public void setMessageTranslatorUtility(
			MessageTranslatorUtility messageTranslatorUtility) {
	...
	public CreateCouponResponse createCoupons(CreateCouponRequest request) {
		...
		try {
			List<String> couponsCodes = promotionAdminService.createCoupons(
					request.getCount(), request.getLength(),
					request.getStartsWith(), request.getEndsWith(),
					request.getPromotionId(), request.getType(), limits,
					request.getAlphabetCase(), request.getAlphaNumericType());
			...
			log.error(
					"Promotion Not Found error in create coupon - "
							+ request.getPromotionId(), e);
			...
			log.error(
					"Coupon Code alphabet character and case type combination is invali = ",
					e);
	...
	@Override
	public AssignCouponToUserResponse assignCouponToUser(
			AssignCouponToUserRequest request) {
		...
		try {
			...
			promotionAdminService.assignCouponToUser(request.getCouponCode(),
					user.getUserId(), request.getOverrideCouponUserLimit());
			...
			log.error(
					"Coupon already assigned to user : "
							+ request.getCouponCode(), e);
			...
			log.error(
					"Error while assigning coupon to user. CouponCode : "
	...
	public SearchCouponResponse searchCoupons(SearchCouponRequest request) {
		...
		try {
			searchCouponResultBO = promotionAdminService.searchCoupons(
					request.getCouponCode(), request.getUserName(),
					request.getOrderBy(), request.getSortOrder(),
					request.getStartRecord(), request.getBatchSize());
		...
		if (searchCouponResultBO.getCouponBasicDetailsSet().isEmpty()) {
			log.info("No coupon data found for search criteria coupon code = "
					+ request.getCouponCode() + ", userName = "
					+ request.getUserName());
		...
		// success case
		response.setCouponBasicDetailsList(new ArrayList<CouponBasicDetails>(
				searchCouponResultBO.getCouponBasicDetailsSet()));
	...
	public ViewCouponResponse viewCoupon(ViewCouponRequest request) {
		...
		try {
			if (useCouponID) {
				...
				log.info("Viewing coupon by coupon code = "
						+ request.getCouponCode());
				couponTO = promotionAdminService.viewCoupons(request
						.getCouponCode());
			}
