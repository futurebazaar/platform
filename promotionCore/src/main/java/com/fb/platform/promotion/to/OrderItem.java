...
public class OrderItem {
	...
	public boolean isApplicableToOrderItem(OrderItem orderItem, List<Integer> brands,
		List<Integer> includeCategoryList, List<Integer> excludeCategoryList, List<Integer> sellerList) {
		if ((!ListUtil.isValidList(brands) || orderItem.isOrderItemInBrand(brands))
			...
			&& (!ListUtil.isValidList(sellerList) || orderItem.isOrderItemInCategory(sellerList))) {
			return true;
		}
		return false;
	}

	public boolean isOrderItemOfSeller(List<Integer> sellers) {
		boolean val = product.isProductOfSeller(sellers);
		return val;
	}
}