...
import com.fb.platform.promotion.rule.config.data.MonthlyDiscountOffRuleData;
...
public class MonthlyDiscountOffRuleImpl implements PromotionRule, Serializable {

	private static transient Log log = LogFactory.getLog(BuyWorthXGetYRsOffRuleImpl.class);

	MonthlyDiscountOffRuleData data = new MonthlyDiscountOffRuleData();
	...
	public void init(RuleConfiguration ruleConfig) {
		...
		data = (MonthlyDiscountOffRuleData) RulesEnum.MONTHLY_DISCOUNT_OFF.getRuleData(ruleConfig);
	...
	public PromotionStatusEnum isApplicable(OrderRequest request, int userId,  
		...
		int noOfTimesInMonth = request.getNoOfTimesInMonth();
		
		if (   !promotionDao.isNoOfTimesInMonth(userId)  ) {
	...
	public RuleConfigMetadata getRuleConfigMetadata() {
		// TODO Auto-generated method stub
		return new MonthlyDiscountOffRuleMetadata();
	}
