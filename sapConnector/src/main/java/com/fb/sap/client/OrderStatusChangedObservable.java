...
class OrderStatusChangedObservable extends Observable implements ISAPEvent {

	private static OrderStatusChangedObservable INSTANCE = new OrderStatusChangedObservable();
	static Logger logger = Logger.getLogger("auris");
	
	public static OrderStatusChangedObservable getInstance() {
		return INSTANCE;
	}
	...
	public synchronized void dispatch(String xml) {
		...
		try {
			JAXBContext context = JAXBContext
					.newInstance("com.fb.sap.idocs");
			Unmarshaller unmarshaller = context.createUnmarshaller();
			OrderIDoc OrderIDoc = (OrderIDoc) unmarshaller
					.unmarshal(new StreamSource(new StringReader(xml)));
			this.setChanged();
			this.notifyObservers(OrderIDoc);
