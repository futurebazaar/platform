...
public final class Money implements Comparable<Money>, Serializable {
	 ...
	 * 
	 * @param aMoneys
	 *            collection of <tt>Money</tt> objects, all of the same
	...
	public Money div(double aDivisor) {
		BigDecimal newAmount = fAmount
				.divide(asBigDecimal(aDivisor), fRounding);
		return new Money(newAmount, fCurrency, fRounding);
	}
	...
	private void validateState() {
		...
		if (fAmount.scale() > getNumDecimalsForCurrency()) {
			throw new IllegalArgumentException("Number of decimals is "
					+ fAmount.scale() + ", but currency only takes "
	...
	/*
	 * private void checkCurrenciesMatch(Money aThat){ if (!
	 * this.fCurrency.equals(aThat.getCurrency())) { throw new
	 * MismatchedCurrencyException( aThat.getCurrency() +
