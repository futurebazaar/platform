...
import com.fb.platform.egv.mapper.ActivateMapper;
...
public class GiftVoucherResource {
	...
	public String rollbackUse(String rollbackUseRequestXml) {
		...
		try {
			com.fb.platform.egv.to.RollbackUseRequest useRollbackRequest = RollbackUseMapper
					.xmlToCoreRequest(rollbackUseRequestXml);

			com.fb.platform.egv.to.RollbackUseResponse useRollbackResponse = giftVoucherManager
					.rollbackUse(useRollbackRequest);
	...
	@Consumes("application/xml")
	@Produces("application/xml")
	public String activate(String activateRequestXml) {
		if (logger.isDebugEnabled()) {
			logger.debug("ActivateRequestXML : \n" + activateRequestXml);
		}
		try {
			com.fb.platform.egv.to.ActivateRequest activateRequest = ActivateMapper
					.xmlToCoreRequest(activateRequestXml);

			com.fb.platform.egv.to.ActivateResponse activateResponse = giftVoucherManager.activate(activateRequest);

			String activateResponseXml = ActivateMapper.coreResponseToXml(activateResponse);

			if (logger.isDebugEnabled()) {
				logger.debug("Activate Response XML :\n" + activateResponseXml);
			}

			return activateResponseXml;

		} catch (JAXBException e) {
			logger.error("Error in the activate GiftVoucher call : ", e);
			return "error"; // TODO return proper error response
		} catch (DatatypeConfigurationException e) {
			logger.error("Error in the activate GiftVoucher call : ", e);
			return "error"; // TODO return proper error response
	...
	@Consumes("application/xml")
	@Produces("application/xml")
	public String sendPin(String sendPinRequestXml) {
		if (logger.isDebugEnabled()) {
			logger.debug("SendPinRequestXML : \n" + sendPinRequestXml);
		}
		try {
			com.fb.platform.egv.to.SendPinRequest sendPinRequest = SendPinMapper.xmlToCoreRequest(sendPinRequestXml);

			com.fb.platform.egv.to.SendPinResponse sendPinResponse = giftVoucherManager.sendPin(sendPinRequest);

			String sendPinResponseXml = SendPinMapper.coreResponseToXml(sendPinResponse);

			if (logger.isDebugEnabled()) {
				logger.debug("SendPin Response XML :\n" + sendPinResponseXml);
			}

			return sendPinResponseXml;
