...
public class CryptoKeyUtils {
	...
	/**
	 * Turns the key data into a key object. The encodedKey is a 48 character
	 * hexadecimal number represting 24 bytes of dey data e.g.
	 * 5A5FB40D45E4968BF883163919B0DC9CD751DAB709EC43B1
	 * 
	 * @param encodedKey
		...
		try {
			...
			if (encodedKey != null) {
				...
				if (isCBCEncryptionModeEnabled) {
					key = SecretKeyFactory.getInstance(NEW_CIPHER_TRANSFORM_TYPE).generateSecret(
							new DESedeKeySpec(bytes));
				} else {
					key = SecretKeyFactory.getInstance(OLD_CIPHER_TRANSFORM_TYPE).generateSecret(
							new DESedeKeySpec(bytes));
				}
			} else {
			...
			logger.error("Failed to decode key data " + encodedKey, e);
	...
	/**
	 * Decrypt the cipher text using current key, if that is unsuccessfull use
	 * old key, if that is unsuccessfull use old key
	 * 
	 * @param cipherText
	 * @param keys
	...
	public static String decrypt(String cipherText, CryptoKeysTO keys) {
		...
		try {
			...
			try {
				...
				// This occurs if the text was encrypted with a key we no longer
				// have
			}
			...
			// test the edge case where the token has been encrypted with the
			// next key
			try {
	...
	public static boolean isCBCEncryptionModeEnabled() {
		...
		// ConfigEntryEnum.IS_CBC_ENCRYPTION_MODE_ENABLED
		return false;
	}

	/**
	 * Append equal sign('=') to the <code>cipherText</code> if required. This
	 * required because a new version of Tomcat(5.5.28 onwards) doesn't allow
	 * '=' to be presented in a cookie value and strip it out.
	 * 
	 * @param cipherText
	 *            a cipher text
